...
vec4 pointLight(float ambientIntensity,int power, float distanceModifier){
	...
	return (texture(diffuse0, texCoord) * lightColor * (diffuse + ambient)  + texture(diffuse0,texCoord).r*specular)/(Distance);
}
...
{
	// outputs final color
	//FragColor = pointLight(0.3f,8,0.00004f);
	float depth = 1-logisticDepth(gl_FragCoord.z);
	FragColor = directionalLight(0.1, 10)*depth;
}